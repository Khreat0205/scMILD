...
class VQ_AENB_Conditional(nn.Module):
    ...
    Study/Batch information is injected into both Encoder and Decoder (scVI style).
    ...
    def __init__(self, input_dim, latent_dim, device, hidden_layers, 
                 ...
                 num_codes=256, commitment_weight=0.25, activation_function=nn.ReLU):
        super(VQ_AENB_Conditional, self).__init__()
        self.device = device
        self.latent_dim = latent_dim
        self.input_dim = input_dim
        self.epsilon = 1e-4
    ...
    def encoder_forward(self, x, study_ids):
    ...
    def forward(self, x, study_ids, y=None, is_train=True):
        """
    ...
    def get_codebook_indices(self, x, study_ids):
        """
        Get codebook indices for input data.
    ...
    def init_codebook(self, dataloader, method="kmeans", num_samples=10000):
        ...
        with torch.no_grad():
            for batch in dataloader:
                data = batch[0].to(self.device)
                study_ids = batch[1].to(self.device) if len(batch) > 1 else torch.zeros(data.shape[0], dtype=torch.long, device=self.device)
